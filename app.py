...
try:
    os.makedirs(app.config['UPLOAD FOLDER'])                                         #creating the Proj2_Python folder if it doesn't exist already
except:
    pass
...
def index():
    if request.method == "POST":
        ...
        if len(list) != 0:                                                          #checking if the size of list is non-zero
            flash("following roll nos don't exist")                                 #that is if all roll nos exist
            flash(list)
